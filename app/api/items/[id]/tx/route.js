...
// 출고: 가용재고 검사 후 outbound에 적재(재고 -qty)
// 입고(반납): outbound에서 차감(재고 +qty)
// 폐기: outbound에서 차감하되 재고로 복귀하지 않고 총 보유수량 자체를 차감
export async function POST(req, { params }) {
  const { id } = await params;
  const { type, qty: rawQty, staff = '', popup } = await req.json();
  const qty = parseInt(rawQty) || 0;
  if (!['출고', '입고', '폐기'].includes(type)) return Response.json({ error: '잘못된 유형' }, { status: 400 });
  ...
  await col.updateOne({ _id: it._id }, { $set: { outbound, history, totalStock: total, updatedAt: new Date() } });
  const { _id, ...rest } = it;
  return Response.json({ id: _id.toString(), ...rest, outbound, history, totalStock: total });
}
